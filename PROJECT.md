...
用户在 `v0.1.0-beta.8` 完成捕获并点击“调用一次 AI 识别”后，收到“模型接口响应超时”。源码确认实验请求的读取时限固定为 60 秒，而 P2 最多会向模型发送 12 条、每条 8,000 字符的捕获片段，最坏接近 10 万字符；较慢的中转模型可能在时限内无法完成。

### 修复边界

- 本机仍保留最多 12 条脱敏候选用于最终字段核对，不降低本机验证真实性。
- 发送给 AI 的候选缩减为优先级最高的 8 条，每条最多 4,000 字符；页面文字最多 4,000 字符，减少推理输入。
- 模型读取时限从 60 秒调整为 120 秒，连接时限仍为 20 秒；不自动重试、不无限等待。
- 调用期间明确提示通常需要等待，不再让正常推理看起来像界面卡死。
- 超时时提示“120 秒内未完成”，建议稍后重试或换响应更快的模型，不暴露技术异常。

### 明确不做

- 不改变已验证字段与页面观察的判定规则。
- 不保存映射、不重放网页请求、不接入 Widget。
- 不修改现有 Widget、Adapter、账户授权或缓存。
- 不新增并发调用或后台自动重试。

### 验收标准

- 同一站点、同一模型不再因原 60 秒固定时限提前失败。
- AI 请求输入明显小于 beta.8，同时本机仍可使用完整候选核对 AI 返回路径。
- 超过 120 秒仍无响应时明确结束并允许用户再次操作，不无限卡住。
- GitHub Actions 只执行一次 `testDebugUnitTest + assembleDebug`，发布固定签名 `v0.1.0-beta.9`。
...
用户在 `v0.1.0-beta.9` 已成功完成模型识别，说明原 60 秒超时不再提前终止本次链路。模型从 MiMo 的真实 `/api/v1/usage` 响应识别出 12 个字段，但返回 `data.costUsage.totalCost` 这类省略根标记 `$` 的点路径；本机验证器只接受 `$.data.costUsage.totalCost`，因此 12 项全部被误判为“字段路径格式不安全”。

### 修复边界

- 接受由普通属性、点号和数组下标组成的相对路径，例如 `data.items[0].quota`。
- 在本机先规范化为标准 JSONPath，例如 `$.data.items[0].quota`，再走原有安全解析和真实取值核对。
- 结果输出统一保存规范化后的 `$` 根路径，避免后续规则格式不一致。
- 递归、通配符、过滤器、函数、脚本和反斜线表达式仍然拒绝，不能因为兼容相对路径降低安全边界。
- 提示模型优先返回以 `$.` 开头的标准路径，但本机验证不依赖模型完全遵守格式。

### 明确不做

- 不放宽 endpoint 必须来自本次捕获的规则。
- 不跳过真实字段取值或实际类型核对。
- 不修改超时、捕获条数、Widget、Adapter、授权或缓存。
- 不保存映射、不重放网页请求、不接入 Widget。

### 验收标准

- `data.costUsage.totalCost` 能规范化为 `$.data.costUsage.totalCost` 并在真实响应存在时通过。
- 已支持的标准 `$` 路径和带引号属性路径保持可用。
- 相对形式的通配符、过滤器、递归和函数表达式继续判定为不安全。
- GitHub Actions 只执行一次 `testDebugUnitTest + assembleDebug`，发布固定签名 `v0.1.0-beta.10`。
...
用户网页登录并捕获真实 JSON
→ AI 只负责识别一次字段语义
→ App 本机核对 endpoint、JSON 路径和值类型
→ App 使用当前网页登录 Cookie 对已验证 GET 接口执行一次直接请求
→ 直接请求仍能取得全部已验证字段后，才允许保存配方
→ 后续从实验室直接刷新已保存配方，不再调用 AI，也不要求重新打开网页

本阶段只证明“简单网页账户接口可以从捕获转为可重复直连”，不承诺所有网站协议都能自动重放。
...
- 页面用途和仪表盘 HTTPS 地址
- 与当前确认页面同 Origin 的 HTTPS endpoint
- 固定请求方法 `GET`
- 已验证指标的类型、名称、JSON 路径、值类型和单位
- 创建时间与最近一次成功直连时间
- 加密 Cookie 的本机引用

配方不保存捕获响应正文、页面全文、AI API Key、样本值或模型原始输出。Cookie 必须使用 Android Keystore 加密后写入本机；加密失败不得降级为明文保存。

### 直接请求安全边界

1. 首版只接受 `GET`；`POST`、GraphQL、请求体和动态 CSRF 明确不支持。
2. endpoint 必须来自本次真实捕获、没有查询参数，并且与用户确认页面属于同一个精确 HTTPS Origin。
3. 只允许 2xx JSON 响应；重定向、HTML、空响应和超出大小上限的响应均不得保存为成功配方。
4. 直接请求使用当前 endpoint 对应的 WebView Cookie，并只发送最小请求头：`Cookie`、`Accept`、`Origin`、`Referer` 和固定移动端 `User-Agent`。
5. 多个 endpoint 按 Host 串行请求；同 Host 请求间隔不少于 500ms，不自动重试。
6. 每个已验证字段必须在直接请求的新响应中再次真实取到，且实际值类型与识别结果一致；任意字段失败则整份配方不保存。
7. 后续刷新发现 Cookie 失效、HTTP 错误、字段缺失或类型变化时，保留配方但明确提示需要重新登录或重新识别，不把旧值冒充新值。
8. 实验室退出时仍清理 WebView Cookie、网页存储和捕获正文；只有用户主动点击并通过“直接测试并保存”后，加密凭据和最小配方才可保留。

### 本阶段范围

- 在现有“仪表盘识别实验室”结果页增加“直接测试并保存”。
- 在实验室配置页显示最近保存的一份配方，并支持无需 AI 的“直接刷新”和“删除配方”。
- 直接刷新结果区分真实取得的字段值和明确错误，不输出 Cookie 或技术堆栈。
- 新增配方规则、持久化和直接请求的单元测试。
- 使用固定 Beta 签名发布新的公开 Debug Prerelease。

### 明确不做

- 不接入现有 Widget、AdapterFactory、最近成功缓存或主 App 槽位配置。
- 不替换 MiMo、DeepSeek 等已经稳定工作的专用 Adapter。
- 不支持跨 Origin endpoint、带查询参数接口、POST、GraphQL、OAuth Token、Bearer Token 或 localStorage 凭据重放。
- 不保存多份站点配方；首版只保留最近一份已验证配方。
- 不在后台定时刷新，不上传开发者服务器，不合并到 `main`。

### 验收标准

- 没有通过 AI 本机核对、没有登录 Cookie、存在非 GET/跨 Origin/带查询参数 endpoint 时，“直接测试并保存”必须拒绝并说明原因。
- 对 MiMo 这类同 Origin、无查询参数、GET + Cookie JSON 接口，直接测试能够在不再次调用 AI 的情况下取得全部已验证字段。
- 保存后关闭并重新打开实验室，能够直接刷新配方并展示本次新请求取得的值。
...
仪表盘识别实验室不再要求用户手动输入模型名称。用户只填写 OpenAI-Compatible API Base 和 API Key，由 App 真实请求模型列表并让用户选择，避免模型名称拼写错误或选择不存在的模型。

### 实现规则

1. 复用主 App 已有的标准模型检测规则，根据 API Base 请求 `/v1/models`，认证使用用户本次输入的 API Key。
2. 请求只执行一次，不自动重试；连接与读取超时沿用现有模型检测的 10 秒限制。
3. 只接受标准 `data[].id` 模型列表；单模型自动选中，多模型通过下拉列表由用户明确选择。
4. API Base 或 API Key 任一发生变化，立即清除旧模型列表和选择；旧异步请求返回时不得覆盖新输入。
5. 未完成真实模型检测并选择模型前，不允许打开仪表盘或开始后续 AI 识别。
6. API Key 与模型选择仍只保存在当前实验 Activity 内存，不写入实验配方、日志或 GitHub。
7. 401/403、404、429、5xx、超时和网络错误使用现有中文分类提示，不向普通用户展示响应正文或堆栈。

### 验收标准

- 实验室配置页不再出现可手工编辑的“模型名称”输入框。
- 填写有效 API Base 和 API Key 后，点击“测试连接并获取模型”能够显示真实模型列表。
- 选择一个模型后才能点击“打开仪表盘并登录”，后续 AI 识别使用所选模型。
...
## Stage 8F-P5-U：通用第三方仪表盘请求配方引擎（用户已确认核心目标）

### 产品目标

用户不需要知道站点接口、请求方法、请求头或 JSON 路径。用户只需要填写模型接口、登录第三方仪表盘、打开目标页面并确认捕获；App 自动判断该站点应使用“直接重放”还是“网页登录辅助刷新”，把真实字段映射到模型实例卡片。

本阶段的“通用”指不按域名新增 Adapter，并覆盖常见第三方仪表盘使用的 GET、带查询参数 GET、POST JSON、POST Form、Cookie、Bearer/API Token、CSRF、用户/租户路由头和短期动态授权。不得宣传为能够绕过验证码、硬件证明、设备指纹、强反爬、证书绑定或站点主动阻止自动化。

### 两级执行模式

1. **直接重放模式**：从用户确认后真实成功的请求中生成最小配方，保存方法、完整 endpoint、允许的查询参数、有限 JSON/Form 请求体、必要认证头和响应字段映射；认证与请求体中的敏感值整体使用 Android Keystore 加密。保存前必须立即真实重放并核对全部字段。
2. **网页登录辅助模式**：当请求依赖一次性签名、Service Worker、短期 Token 或无法安全导出的浏览器状态时，不伪造直连配方；刷新时重新进入已批准站点的 WebView，由页面自己产生合法请求，App 只捕获并本机核对响应。登录过期时明确要求用户重新登录。

### 安全边界

1. 只捕获用户明确确认的 HTTPS 仪表盘页面，以及用户本次页面真实访问过并再次确认的数据 API Origin；不允许 AI 虚构 endpoint 或扩大 Origin。
2. AI 只接收脱敏响应结构和页面文字；Cookie、认证头、查询参数敏感值、请求体敏感值、localStorage/sessionStorage 内容不得进入 AI Prompt、日志、GitHub 或开发者服务器。
3. 只保存本机验证确实需要的最小请求字段；Host、Origin、Referer、User-Agent 和浏览器安全头由客户端生成，不照抄任意网页值。
4. 只支持有限大小的 JSON/Form 请求体；文件上传、multipart 二进制、流式请求、WebSocket 和任意脚本执行不进入直接重放模式。
5. 每份配方绑定一个模型实例和独立缓存身份；失败不得覆盖最近成功数据，不得把网页观察文字冒充接口数据。
6. 发现验证码、设备证明、动态签名无法重放或认证被拒绝时，自动降级到网页登录辅助模式并显示原因，不无限重试。

### 分阶段实现顺序

1. **P5-U1**：把现有配方从“无查询 GET”扩展为真实捕获的 GET、带查询参数 GET、POST JSON/Form；完成请求体脱敏分类、Keystore 加密、真实二次核对和回归测试。只做直接重放，不接 Widget。
2. **P5-U2**：支持用户确认的页面 Origin 与数据 API Origin 分离，建立精确 Origin 允许清单与重定向规则。
3. **P5-U3**：实现网页登录辅助刷新，用于动态签名、Service Worker 和短期授权；明确前台刷新与登录失效行为。
4. **P5-U4**：统一接入 AdapterFactory、模型实例缓存与 Widget；按配方能力自动选择直接重放或网页登录辅助模式。

每个子阶段必须分别完成 GitHub Actions 构建、固定签名发布和用户真机验收；前一阶段未验收不得进入下一阶段。

### P5-U1 验收标准

- 同一套代码可对至少三个不同实现的真实第三方站点生成配方，不按域名新增 Adapter。
- GET、带查询参数 GET、POST JSON/Form 各有规则测试；至少一个真实 POST 或带查询参数站点完成真机二次核对。
- 点击任何操作均立即有可见状态；所有拒绝和失败路径给出中文原因并允许重试。
- 认证信息和敏感请求参数只存在于内存或 Keystore 密文，公开仓库与 AI 请求中不存在真实值。
- 无法安全直接重放的站点明确标记“需要网页登录辅助刷新”，不保存一个注定失败的配方。

### P5-U1.1：模型 5xx 状态与原地重试反馈

- beta.18 真机调用 AI 识别时收到“模型服务暂时异常”。源码确认该文案把 `/v1/chat/completions` 返回的全部 HTTP 500～599 合并，用户无法判断是内部错误、网关异常、服务不可用还是上游超时。
- 仅公开安全的 HTTP 状态分类：500、502、503、504 分别给出中文原因，其他 5xx 显示具体状态码；不得展示模型服务原始错误正文、请求内容、API Key 或捕获数据。
- 失败后保留当前网页捕获，恢复同一按钮并明确显示“重新调用一次 AI 识别”；用户可原地手动重试，不自动重试、不重复调用模型。
- 本修复不改变 AI Prompt、捕获范围、请求配方、Keystore、Widget、Adapter、缓存或 P5-U1 协议边界。
- beta.19 必须通过 JVM、Lint、编译、beta.18 覆盖安装、全部 Android Instrumentation/UI Automator 和固定签名门禁后才可发布。

---

## Stage 8F-QA-1：Public Beta 全链路审计与自动化发布门禁（用户已确认）

### 目标

在继续 P5-U 通用配方能力前，对当前主 App、网页登录、模型检测、平台 Adapter、配置与凭据持久化、最近成功缓存、Widget、仪表盘实验室和 GitHub 发布链路做一次集中稳定性收口。用户明确要求本轮自行测试到位；因此新 APK 只有在单元测试、Android Lint、模拟器覆盖安装和跨进程 UI 自动化全部通过后才允许交付。

### 必须收口

1. 历史平台公共授权迁移只能读取精确公共键，不得通过兼容别名把某个槽位的凭据复制给其他槽位；迁移写入失败时保留旧公共凭据。
2. 服务识别只按解析后的真实 Host 判断，网址路径、查询文字和相似域名不得触发平台 Adapter 或能力说明。
3. 模型检测与携带 Cookie/Token/API Key 的请求禁止自动跨重定向发送认证；连接必须在成功、错误和异常路径全部关闭。
4. 模型接口的错误详情不展示服务端原始响应正文；任意第三方模型列表响应设置本机大小上限，避免异常响应无限占用内存。
5. 网页登录 WebView 禁止文件与 Content Provider 访问、混合内容和多窗口，并启用系统安全浏览能力。
6. 最近成功缓存只保存真实指标；仅有模型名称的空成功不得写入缓存，残缺结果不得覆盖同实例已有完整结果，不同实例不得串读。
7. Public 仓库当前文件树移除旧源码备份和含本机 `local.properties` 的项目压缩包，补充 `.env`、签名、凭据与备份文件忽略规则。

### 自动化发布门禁

- JVM：全部规则、字段映射和 URL 安全测试通过。
- 静态：`lintDebug` 零阻断错误，`assembleDebug` 成功。
- Android 模拟器：先安装上一固定签名 Beta，再用 `adb install -r` 覆盖当前 APK并成功启动。
- Instrumentation：真实点击“直接测试并加密保存”按钮，任何失效状态都必须立即出现可见原因和重试按钮。
- Instrumentation：API Key 加密持久化可回读、平台公共凭据精确迁移、完整缓存不被残缺/空结果覆盖、不同实例缓存隔离全部通过。
- 签名：APK 证书 SHA-256 必须继续等于项目固定 Beta 公开指纹；上述任一门禁失败时不得发布或提供安装命令。

### 证据边界

自动化可以证明固定输入、模拟器和云端构建链路通过，但不能宣称覆盖所有第三方站点、OEM Launcher、VPN 或真实账户状态。最终 APK 仍需用户做一次真机网页登录、Widget 和目标站点验收；发现问题后按真实证据继续修复，不宣称绝对“零 Bug”。
