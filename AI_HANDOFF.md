...
`Stage 8F-P5-U1：通用 GET/查询 GET/POST JSON/Form 直接重放`
...
- P5-U1 当前状态：P5-U1.1 已实现 500/502/503/504 安全状态分类、捕获保留与原地手动重试反馈；Release 目标 `0bf18a9050d32ee4e65ace0394de1a6312833f36` 的 Actions `29686369540` 全门禁通过并发布 beta.19，等待用户在原真实站点复测
...
用户安装 beta.19 后在同一真实站点原地重试 AI 识别，再继续“直接测试并加密保存 → 关闭重开实验室 → 直接刷新”。若模型服务仍失败，记录 beta.19 显示的具体 500/502/503/504；P5-U1 真机通过前不进入跨 Origin P5-U2。
...
## Stage 8F-P5-A.1 保存按钮反馈修复

- 用户提供的 PuppyRouter 结果包含两个同源、无查询参数 GET 接口和 9 个本机验证字段，符合现有配方规则；该识别结果本身不是空结果。
- 源码确认按钮处理存在 `recipeInProgress`、`currentAnalysis` 和 `currentCapture` 三个无提示直接返回路径；合法请求开始后按钮文字也不变化，用户无法判断点击是否生效。
...
- 本条为 QA-1 当时状态：用户先要求完成全功能与数据逻辑审计，P5-U1 在该批次冻结；beta.17 真机通过后用户已另行批准开始 P5-U1。
